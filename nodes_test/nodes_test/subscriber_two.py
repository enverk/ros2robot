...
# ikinci yayınını dinleyen abonenin kodları yazılmaktadır.
class Subscriber_Two(Node):
    def __init__(self):
        super().__init__("Subscriber_Two")
        ...
        self.subscription = self.create_subscription(
            String, "publication_two", self.listener_callback, 10
        )
...
def main(args=None):
    ...
    subscriber_two = Subscriber_Two()
    rclpy.spin(subscriber_two)

    # Düğüm sonlandığında Düğüm silinmiştir.
    subscriber_two.destroy_node()
    rclpy.shutdown()
